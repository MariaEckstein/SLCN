...
all_files_sum_pre = ddply(subset(all_files, !is.na(choice_12_back)),
                      .(sID, version, age_group, Columbia_cat, model_name, outcome_21_back, choice_12_back), summarize,
                      choice_left = mean(choice_left, na.rm = T))
all_files_sum = ddply(all_files_sum_pre,
                      .(sID, age_group, Columbia_cat, model_name, outcome_21_back, choice_12_back), summarize,
...
# ACC over blocks
ACC_subj = ddply(all_files, .(sID, trialsinceswitch, age_group, model_name, PreciseYrs, Columbia_cat), summarize, ACC = mean(ACC, na.rm=T))  # to make sure error bars are right
gg_ACC_blocks =
  ggplot(ACC_subj, aes(trialsinceswitch, 100 * ACC, group = age_group, color = age_group, shape = age_group)) + 
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "pointrange") +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "line") +
  geom_vline(xintercept = 0, linetype = "dotted") +
...
gg_ACC_blocks_PDS = gg_ACC_blocks + aes(group=Columbia_cat, color=Columbia_cat, shape=Columbia_cat)
...
reward_subj = ddply(subset(all_files, !is.na(outcome_21_back)),
                    .(sID, Columbia_cat, outcome_21_back, age_group, model_name, PreciseYrs), summarize, stay = mean(same_choice_01_back, na.rm = T))
...
stay_plot =
  ...
  facet_grid(~ model_name)
stay_plot_PDS = stay_plot + aes(fill=Columbia_cat, group=Columbia_cat)
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "/gg_ACC_blocks_PDS.png"), gg_ACC_blocks_PDS, width = 3.3, height = 1.4 + length(unique(gg_ACC_blocks$data$model_name)))
...
# gg_age_RT =
  ggplot(subset(median_RT, PreciseYrs<24), aes(PreciseYrs, RT, color=log(T1))) + geom_point() + geom_smooth()
# gg_T_RT =
  ggplot(subset(median_RT, PreciseYrs<24), aes(log(T1), RT, color=Gender)) + geom_point() + geom_smooth()

# gg_age_RT =
  ggplot(subset(median_RT, PreciseYrs>=11 & PreciseYrs<=14), aes(PreciseYrs, RT, color=log(T1))) + geom_point() + geom_smooth()
# gg_T_RT =
  ggplot(subset(median_RT, PreciseYrs>=11 & PreciseYrs<=14), aes(log(T1), RT, color=Gender)) + geom_point() + geom_smooth()

mean_ACC = ddply(all_files, .(sID, PreciseYrs, T1), ACC=mean(ACC), T1=median(T1), Gender=median(Gender), summarize)
# gg_age_ACC =
  ggplot(subset(mean_ACC, PreciseYrs<24), aes(PreciseYrs, ACC, color=T1)) + geom_point() + geom_smooth()
# gg_T_ACC =
  ggplot(subset(mean_ACC, PreciseYrs<24), aes(T1, ACC, color=Gender)) + geom_point() + geom_smooth()
  
# gg_T_PDS =
  ggplot(subset(all_files, PreciseYrs<20), aes(PDS, T1, color=PreciseYrs, shape=factor(Gender))) + geom_point()

# z-score predictors
# Calculate regression models
reg_dat = subset(all_files, age_group!="Adults")
...
summary(lmer(log_RT ~ PreciseYrs_z * T1_z + (1 | sID), subset(reg_dat, ACC==T & Gender==2), na.action=na.omit, REML=F, verbose=T))
summary(lmer(log_RT ~ PreciseYrs_z * T1_z + (1 | sID), subset(reg_dat, ACC==T & Gender==1), na.action=na.omit, REML=F, verbose=T))
summary(glmer(ACC ~ PreciseYrs_z * T1_z + (1 | sID), subset(reg_dat, Gender==1), binomial))
summary(glmer(ACC ~ PreciseYrs_z * T1_z + (1 | sID), subset(reg_dat, Gender==2), binomial))
...
summary(all_files$Columbia_cat)
```
