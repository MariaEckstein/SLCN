...
parameter_names = c("alpha", "beta", "epsilon", "forget", "LL")

gg_save = T
data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AlienGenRec/HierarchicalAlphaBeta/0RandomSimulations/NIter20/"
...
if (read_or_create_genrec == "create") {
  ...
  for (filename in filenames) {
    ...
    genrec_row = subset(subj_file,
                        trial_index == max(subj_file$trial_index),  # Get last row -> compare LL and LL_rec
                        select = c("sID", "learning_style", "mix_probs", "fit_pars", "BIC_rec", "AIC_rec", parameter_names, paste0(parameter_names, "_rec")))
...
for (par in parameter_names) { #intersect(unique(plot_dat$fit_par), unique(plot_dat$parameter))) {
  ...
  if (nrow(plot_dat) > 0) {
    gg_gen_rec_par = ggplot(plot_dat, aes(value_gen, value_rec, color = sID)) +# & fit_par == par
      ...
      facet_grid(mix_probs ~ learning_style)
