...
gg_save = T
data_dir = "C:/Users/maria/MEGAsync/Berkeley/TaskSets/AlienGenRec/2018_05_08FitOneParamAtATime"
...
genrec_long = merge(subset(genrec, select = c("sID", "learning_style", "method", "mix_probs", "fit_par")), genrec_long)
...
for (par in parameter_names) { #intersect(unique(plot_dat$fit_par), unique(plot_dat$parameter))) {
  plot_dat = subset(genrec_long, parameter == par & fit_par == par)
  if (nrow(plot_dat) > 0) {
    gg_gen_rec_par = ggplot(plot_dat, aes(value_gen, value_rec, color = mix_probs)) +# & fit_par == par
      geom_point() +
