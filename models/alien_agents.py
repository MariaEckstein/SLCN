...
class Agent(object):
    def __init__(self, agent_stuff, all_params_lim, task_stuff=np.nan):
        ...
        self.select_max = self.learning_style == 'flat'  # Hack to select the right TS each time for the flat agent
    ...
    def select_action(self, stimulus):
        ...
        self.p_TS = self.get_p_from_Q(self.Q_high[stimulus[0], :], [], self.select_max)  # works for flat & hierarchical
        p_actions = [self.get_p_from_Q(self.Q_low[TS_i, stimulus[1], :], self.prev_action) for TS_i in range(self.n_TS)]
    ...
    def get_p_from_Q(self, Q, previous_choice, select_max=False):
        Q[previous_choice] += self.perseverance  # Should affect the original Q table! -> Perseverance is persistent!
        if select_max:
            p = Q == 1  # select the one TS that has a value of 1 (all others have values of 0)
    ...
    def update_Qs(self, stimulus, action, reward):
        RPEs_low = reward - self.Q_low[:, stimulus[1], action]
        RPEs_high = reward - self.Q_high[stimulus[0], :]
