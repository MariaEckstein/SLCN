...
def get_phigh_series(Q_high_series, season_series, beta_high, n_subj):
    return [softmax(beta_high * Q_high[season]) for Q_high, season in zip(Q_high_series, season_series)]


def get_plow_series(Q_low_series, TS_series, alien_series, beta, n_subj):
    return [softmax(beta * Q_low[TS, alien]) for Q_low, TS, alien in zip(Q_low_series, TS_series, alien_series)]
...
def generate_TS_series(TS_series, n_TS):
    ...
    TS_series_new = np.array([np.random.randint(0, n_TS, len(TS_series))])
...
def compute_logprob(TS_series, season_series, alien_series, action_series, reward_series,
                    ...
                    n_subj, n_TS, n_aliens, n_actions):
...
def sample_TS_series(season_series, alien_series, action_series, reward_series,
    ...
    for i in range(n_samples_MCMC):
        ...
        TS_series_new = generate_TS_series(TS_series, n_TS, n_subj)
    ...
    return TS_series_samples, L_samples, accepted_samples  # List of lists, one for each sample
