...
from AlienSimulation import do_simulate
...
from shared_aliens_bas import get_alien_paths, read_in_human_data
...
# Get simulated data
(alphas, betas, forgets, alpha_highs, beta_highs, forget_highs), (season_series, TS_series_true, alien_series, action_series, reward_series) = do_simulate()
alpha, beta, forget, alpha_high, beta_high, forget_high = alphas.flatten()[subj_id], betas.flatten()[subj_id], forgets.flatten()[subj_id], alpha_highs.flatten()[subj_id], beta_highs.flatten()[subj_id], forget_highs.flatten()[subj_id]
...
def test_MCMC():
    start = time.time()
    TS_series_samples, L_samples, accepted_samples = sample_TS_series(
        season_series[:, subj_id], alien_series[:, subj_id], action_series[:, subj_id], reward_series[:, subj_id],
...
plt.figure()
plt.title("Percentage repeated TS")
plt.plot([np.mean(t[:-1]==t[1:]) for t in TS_series_samples])

plt.figure()
plt.title("Seasons and TS")
plt.plot(TS_series_samples[0,:])
plt.plot(np.mean(TS_series_samples[int(n_samples_MCMC/2):,:],axis=0),'.')

plt.figure()
plt.title("Correlation between season and TS")
plt.plot([np.corrcoef(season_series[:,subj_id],TS_series_samples[i,:])[0,1] for i in range(n_samples_MCMC)])

plt.figure()
plt.title("Correlation between true and inferred TS")
plt.plot([np.corrcoef(TS_series_true[:,subj_id],TS_series_samples[i,:])[0,1] for i in range(n_samples_MCMC)])

# plt.figure()
# plt.title("Correlation between true and inferred TS switches")
# plt.plot([np.corrcoef(TS_series_true[:,subj_id],TS_series_samples[i,:])[0,1] for i in range(n_samples_MCMC)])
plt.show()
