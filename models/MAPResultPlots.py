...
for file_name in file_names:
    ...
    if (waic < waic_criterion_for_analysis) and (waic > 0):
        ...
        if group_level_param_names and False:  # TODO
            ...
            fitted_params_g.to_csv(save_dir + 'params_g_' + file_name[:-7] + '.csv')
        ...
        plt.savefig("{0}fitted_param_distr_{1}_{2}.png".format(save_dir, model_name, slope_variable))
        plot_fitted_params_against_cols(fitted_params, indiv_param_names, cols=[col + '_quant' for col in cols], type='line')
        plt.savefig("{0}fitted_param_quant_{1}_{2}.png".format(save_dir, model_name, slope_variable))

        # Save correlations
        print("Saving correlations between fitted parameters and age, PDS, T for model {0} to {1}.".format(file_name, save_dir))
        corrs.to_csv("{0}corrs_{1}_{2}.csv".format(save_dir, model_name, slope_variable), index=False)
        ...
        plt.savefig("{0}param_pairplot_{1}_{2}.png".format(save_dir, model_name, slope_variable))

# Plot and save WAICS
waics.columns = ['model_name', 'slope_variable', 'waic']
