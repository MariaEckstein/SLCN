...
adults_only = False
n_groups = 3
n_subj = 6#'all'  # 'all' or int
n_trials = 4#'all'  # 'all' or int
...
n_subj, rewards, choices, group, n_groups, age_z = load_data(run_on_cluster, fitted_data_name, n_groups, kids_and_teens_only, adults_only, verbose, n_subj)
if n_trials != 'all':
...
choices_both = np.transpose(choices_both, (1, 2, 0))[:-1]  # make same shape as Qs (n_trials-1, n_subj, 2)
...
with pm.Model() as model:
    ...
    calpha_sc = pm.Beta('calpha_sc', alpha=calpha_sc_a[group], beta=calpha_sc_b[group], shape=n_subj, testval=0.6 * T.ones(n_subj))
    ...
    alpha = pm.Beta('alpha', alpha=alpha_a[group], beta=alpha_b[group], shape=n_subj, testval=0.8 * T.ones(n_subj))
    b = pm.Gamma('b', alpha=beta_a[group], beta=beta_b[group], shape=n_subj, testval=T.ones(n_subj))
    beta = pm.Deterministic('beta', b + 1)
    # persev = 0
    persev = pm.Normal('persev', mu=persev_mu[group], sd=persev_sd[group], shape=(1, n_subj, 1))#, testval=0.1 * T.ones((1, n_subj, 1)))
    persev_bonus = persev * choices_both

    # gamma = pm.Beta('gamma', alpha=gamma_a[group], beta=gamma_b[group], shape=n_subj)
    nalpha = pm.Beta('nalpha', alpha=nalpha_a[group], beta=nalpha_b[group], shape=n_subj, testval=0.7 * T.ones(n_subj))
    ...
    [Qs, _], _ = theano.scan(
        fn=update_Q,
        sequences=[choices[:-1], rewards[:-1]],  # remove last trial because it's not needed
        outputs_info=[Qs, _],
    ...
    model_choices = pm.Bernoulli('model_choices', p=p_right, observed=choices[1:])  # predict from second trial on
    ...
    if verbose or print_logps:
        ...
        theano.printing.Print('Qs_p')(Qs + persev_bonus)
...
if not run_on_cluster:
    ...
    print("WAIC: {}".format(pm.waic(trace, model)))
