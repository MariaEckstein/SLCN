...
fit_pars = genrec.parameters.par_names
for learning_style in ['RL', 'Bayes']:
    for method in ['direct', 'softmax', 'epsilon-greedy']:
        ...
        agent_stuff['free_par'] = [np.any(par == np.array(fit_pars)) for par in genrec.parameters.par_names]
        genrec.generate_and_recover(fit_pars, agents, n_iter, agent_stuff, task_stuff)
...
parameters = Parameters(par_names=['alpha', 'beta', 'epsilon', 'perseverance', 'decay'],
                        ...
                        # limits for fitting ('fristtry'): ((0, 1), (1, 15), (0, 1), (-1, 1), (0, 1))
                        # limits for fitting: ((0, 1), (1, 10), (0, 0.5), (-0.5, 0.5), (0, 0.5))
                        par_limits=((0, 1), (1, 6), (0, 0.25), (-0.3, 0.3), (0, 0.3)),  # parameter scale
                        default_pars=np.array([0.25, 1+1e-10, 1e-10, 1e-10, 1e-10]))  # parameter scale
genrec = GenRec(parameters)
agent_stuff = {}
...
for learning_style in ['RL', 'Bayes']:
    for method in ['direct', 'softmax', 'epsilon-greedy']:
        ...
        agent_stuff['method'] = method
        agent_stuff['free_par'] = [np.any(par == np.array(fit_pars)) for par in genrec.parameters.par_names]
        genrec.generate_and_recover(fit_pars, range(n_agents), n_iter, agent_stuff, task_stuff, 'generate_and_recover')
        for param_name in genrec.parameters.par_names:
            genrec.vary_parameters(param_name, agent_stuff, task_stuff)
            genrec.generate_and_recover([param_name], range(n_agents), n_iter, agent_stuff, task_stuff, 'generate_and_recover')
