...
plot_sample_composition = function(all_files, gg_save, run_regression) {
  ...
  gg_PDS_composition$data = subset(dat, PreciseYrs < 20)
...
plot_performance_over_age = function(ACCs, gg_save) {
  ...
  # Prepare data
  ACCs$stay_fit = -0.25 -0.586402 + 0.171191 * ACCs$PreciseYrs -0.004136 * ACCs$PreciseYrs ^ 2  # results from summary(glmer(same_choice_01_back ~ PreciseYrs + PreciseYrs2 + Gender + (1 | sID), all_files, binomial)); 10/08/2019; run on all subjects, IDs 12-448, ages 7.77-30
  ACCs$stay_fit[ACCs$Gender == "Female"] = NA  # hack to get rid of double lines
  # ACCs$stay_fit_low = -0.25 -0.586402 + (0.171191 + 1.96 * 0.040369 / n_subj) * ACCs$PreciseYrs -(0.004136 + 1.96 * 0.001079 / n_subj) * ACCs$PreciseYrs ^ 2
  # ACCs$stay_fit_hig = -0.25 -0.586402 + (0.171191 - 1.96 * 0.040369 / n_subj) * ACCs$PreciseYrs -(0.004136 - 1.96 * 0.001079 / n_subj) * ACCs$PreciseYrs ^ 2
  ACCs$stay_fit_low = -0.25 -0.586402 + (0.171191 - 0.040369) * ACCs$PreciseYrs -(0.004136 - 0.001079) * ACCs$PreciseYrs ^ 2
  ACCs$stay_fit_hig = -0.25 -0.586402 + (0.171191 + 0.040369) * ACCs$PreciseYrs -(0.004136 + 0.001079) * ACCs$PreciseYrs ^ 2
  
  ACCs$reward_fit = 35 + 120 * (-0.1233495 + 0.0490270 * ACCs$PreciseYrs -0.0013627 * ACCs$PreciseYrs ^ 2)  # summary(glmer(reward ~ PreciseYrs + PreciseYrs2 + Gender + (1 | sID), all_files, binomial))
  ACCs$reward_fit[ACCs$Gender == "Female"] = NA  # hack to get rid of double lines
  ACCs$reward_fit_low = 35 + 120 * (-0.1233495 + (0.0490270 - 0.0130263) * ACCs$PreciseYrs -(0.0013627 - 0.0003473) * ACCs$PreciseYrs ^ 2)
  ACCs$reward_fit_hig = 35 + 120 * (-0.1233495 + (0.0490270 + 0.0130263) * ACCs$PreciseYrs -(0.0013627 + 0.0003473) * ACCs$PreciseYrs ^ 2)

  ACCs$RT_fit = exp(7.405e+00 -1.694e-01 * ACCs$PreciseYrs + 4.003e-03 * ACCs$PreciseYrs ^ 2)  # summary(lmer(log(RT) ~ PreciseYrs + PreciseYrs2 + (1 | sID), subset(all_files, ACC==T & Gender=="Female")))
  ACCs$RT_fit_hig = exp(7.405e+00 -(1.694e-01 + 2.267e-02) * ACCs$PreciseYrs + (4.003e-03 + 6.082e-04) * ACCs$PreciseYrs ^ 2)
  ACCs$RT_fit_low = exp(7.405e+00 -(1.694e-01 - 2.267e-02) * ACCs$PreciseYrs + (4.003e-03 - 6.082e-04) * ACCs$PreciseYrs ^ 2)
  
  ACCs$RT_fit[ACCs$Gender == "Male"] = exp(7.193e+00 -1.645e-01 * ACCs$PreciseYrs + 3.870e-03 * ACCs$PreciseYrs ^ 2)[ACCs$Gender == "Male"]  # summary(lmer(log(RT) ~ PreciseYrs + PreciseYrs2 + (1 | sID), subset(all_files, ACC==T & Gender=="Male")))
  ACCs$RT_fit_hig[ACCs$Gender == "Male"] = exp(7.193e+00 -(1.645e-01 + 3.470e-02) * ACCs$PreciseYrs + (3.870e-03 + 9.184e-04) * ACCs$PreciseYrs ^ 2)[ACCs$Gender == "Male"]
  ACCs$RT_fit_low[ACCs$Gender == "Male"] = exp(7.193e+00 -(1.645e-01 - 3.470e-02) * ACCs$PreciseYrs + (3.870e-03 - 9.184e-04) * ACCs$PreciseYrs ^ 2)[ACCs$Gender == "Male"]
...
if (gg_save) {
  ggsave(file.path(plot_dir, "gg_beta_RL_BF.png"), gg_beta_RL_BF, width=4, height=4)
  ggsave(file.path(plot_dir, "gg_persev_RL_BF.png"), gg_persev_RL_BF, width=4, height=4)
}
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "gg_genrec_RL.png"), gg_genrec_RL, width=8, height=8)
}
```
