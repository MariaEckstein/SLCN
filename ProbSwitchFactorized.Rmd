...
correlate_params_behavior(param_file_dir, human_data_dir, gg_save) {
  
  # Load model parameters
  params = read.csv(paste0(param_file_dir, "/params_Bbpr_pymc3.csv"))
  ...
  # Create summary data
  ACC_subj = get_ACC_reward_subj(hum_files)[[1]]
  ...
  median_RT = ddply(subset(hum_files, ACC=T), .(sID), RT=median(RT, na.rm=T), summarize)
  
  # Put everything into a single dataframe
  l = list(switch_speed2, switch_speed3, mean_ACC, median_RT)
  cor_dat = subset(params, PreciseYrs < 20, select=c("sID", "beta", "persev", "p_reward", "PDS", "BMI", "T1", "PreciseYrs"))
  for(i in 1:length(l)) {
  ...
  if (gg_save) {
    png(file.path(plot_dir, "/gg_corrplot.png"), width=800, height=800)
...
if (data_name == "simulated_human") {
  ...
  dir_to_all_simulations = 'C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_map_models_RL/simulations'
