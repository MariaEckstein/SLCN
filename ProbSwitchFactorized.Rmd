...
plot_indivduals_for_exclusion = function(all_files, gg_save, exclude_UCB) {
  ...
  age_excl = sort(subset(ACCs, PreciseYrs > 30)$sID)
  if (exclude_UCB) {
    age_excl = c(age_excl, subset(ACCs, sID > 400)$sID)
  }
  ...
  return(unique(c(gender_excl, PDS_excl, n_trials_excl, switch_excl, ACC_excl, age_excl)))
}
```
...
dir_to_all_simulations = 'C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/BF/simulate/simulations'
...
if (data_name == "simulated_human") {
  ...
  for (data_dir in data_dirs) {  # [8:length(data_dirs)]
    data_dir = 'C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/BF/simulate/simulations/Bbpr_age_z_271'
...
plot_parameters = function(fitted_param_dir, plot_appx) {  # , fitted_slopes_dir
  
  # Read in and clean fitted parameter data
  fitted_params = read.csv(fitted_param_dir)
  # fitted_slopes = read.csv(fitted_slopes_dir)
  ...
  # # Plot parmaeters over age
  # int = 1
  # slope = 1
  # slope2 = -1
  # sd = 0.5
  # # gg_params_ages_dist =
  #   ggplot(fp_long, aes(PreciseYrs, value, color=Gender, group=Gender)) +
  #     geom_point() +
  #     stat_function(fun=function(x) int + slope * x + slope2 * x^2, color="black") +
  #     # stat_function(fun=function(x) int + slope * x + slope2 * x^2 + sd, color="grey") +
  #     # stat_function(fun=function(x) int + slope * x + slope2 * x^2 - sd, color="grey") +
  #     facet_grid(parameter ~ ., scales="free")
    
    # ggplot() +
    #   geom_point(data=fp_long, aes(PreciseYrs, value, color=Gender, group=Gender)) +
    #   stat_function(fun=function(x) int + slope * x + slope2 * x^2, color="black",
    #                 data=slopes,
    #                 aes(fp_long$PreciseYrs))
    
  # Plot parameters over age groups
  gg_params_ages_quant =
    ggplot(subset(fp_llong, !is.na(percent) & parameter %in% c("alpha_minus_calpha", "alpha_minus_nalpha", "nalpha_minus_cnalpha")), aes(percent, value, color=Gender, group=Gender)) +
    stat_summary(fun.data=mean_se, geom="pointrange") +
    stat_summary(fun.data=mean_se, geom="line") +
    facet_grid(parameter ~ group, scales="free")
  
  gg_params_ages_quant_a = gg_params_ages_quant + coord_cartesian(ylim=c(0, 1))
  gg_params_ages_quant_a$data = subset(fp_llong, !is.na(percent) & parameter %in% c("alpha", "nalpha", "calpha", "cnalpha"))
  # , aes(percent, value, color=Gender, group=Gender)
    # ggplot(subset(fp_llong, !is.na(percent) & parameter %in% c("alpha", "nalpha", "calpha", "cnalpha")), aes(percent, value, color=Gender, group=Gender)) +
    # stat_summary(fun.data=mean_se, geom="pointrange") +
    # stat_summary(fun.data=mean_se, geom="line") +
    # coord_cartesian(ylim=c(0, 1)) +
    # facet_grid(parameter ~ group, scales="free")
  
  gg_params_ages_quant_b = gg_params_ages_quant + coord_cartesian(ylim=c(1, 11))
  gg_params_ages_quant_b$data = subset(fp_llong, !is.na(percent) & parameter %in% c("beta"))
    # ggplot(subset(fp_llong, !is.na(percent) & parameter %in% c("beta", "persev", "alpha_minus_calpha", "alpha_minus_nalpha", "nalpha_minus_cnalpha")), aes(percent, value, color=Gender, group=Gender)) +
    # stat_summary(fun.data=mean_se, geom="pointrange") +
    # stat_summary(fun.data=mean_se, geom="line") +
    # facet_grid(parameter ~ group, scales="free")
  
  gg_params_ages_quant_p = gg_params_ages_quant + coord_cartesian(ylim=c(-0.25, 0.75))
  gg_params_ages_quant_p$data = subset(fp_llong, !is.na(percent) & parameter %in% c("persev"))
  ...
  for (predictor in c("age_group", "PDS_group", "T_group")) {#
    for (param in param_cols[2:length(param_cols)]) {
      ...
      # print(summary(aov(formula, subset(fitted_params, !age_group %in% c("UCB", "Adult")))))#
  ...
  if (gg_save) {
    ggsave(file.path(plot_dir, paste0("gg_params_ages_dist", plot_appx, ".png")), gg_params_ages_dist, width=4, height=12)
    ggsave(file.path(plot_dir, paste0("gg_params_ages_quant_a", plot_appx, ".png")), gg_params_ages_quant_a, width=8, height=8)
    ggsave(file.path(plot_dir, paste0("gg_params_ages_quant_b", plot_appx, ".png")), gg_params_ages_quant_b, width=8, height=2.5)
    ggsave(file.path(plot_dir, paste0("gg_params_ages_quant_p", plot_appx, ".png")), gg_params_ages_quant_p, width=8, height=2.5)
  }
}

# fitted_slopes_dir = "C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/params_g_RLabcxnplyoqtu_age_z_271_pymc3.csv"
plot_parameters(fitted_param_dir="C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/params_RLabcnpx_age_z_271_pymc3.csv", plot_appx="_ML")
# plot_parameters(fitted_param_dir="C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/params_RLabcxnplyoqtu_age_z_271_pymc3.csv", "_MCMC")
# plot_parameters(fitted_param_dir="C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/params_RLabcnpx_age_z_271_pymc3.csv", "_MCMC_sep")

plot_parameters(fitted_param_dir="C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/params_Bbpr_age_z_271_pymc3.csv", plot_appx="_ML")
# plot_parameters(fitted_param_dir="C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/params_RLabcxnplyoqtu_age_z_271_pymc3.csv", "_MCMC")
# plot_parameters(fitted_param_dir="C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/params_RLabcnpx_age_z_271_pymc3.csv", "_MCMC_sep")
```
