...
library("ggplot2"); theme_set(theme_classic()); library("plyr"); library("reshape2"); library("R.matlab"); library("zoo"); library(lmerTest); library("ggfortify")#; library("corrplot")#; library("Hmisc")
...
c_age_ACC_validation =
  ggplot(ACCs, aes(age_group, mean_ACC, group=model_name, color=model_name)) +
  stat_summary() +
  stat_summary(geom='line')
b_age_ACC =
  ggplot(ACCs, aes(x=age_group_, mean_ACC, color=Gender, group=1)) +
...
b_age_RT =
  b_age_ACC + aes(y=median_cor_RT, group=Gender) +
    ...
    stat_summary(color="black", size=0.2)
...
ggsave(file.path(plot_dir, "c_age_ACC_validation.png"), c_age_ACC_validation, width=4, height=3)
ggsave(file.path(plot_dir, "c_age_stay_validation.png"), c_age_stay_validation, width=4, height=3)
ggsave(file.path(plot_dir, "c_age_reward_validation.png"), c_age_reward_validation, width=4, height=3)
```
```{r Specific measures - Age differences and Model validation}
# "Apparent switch trials" (3rd bar of stay_plot)
e_age_stayappswitch_validation =
...
dat = ddply(subset(ACC_subj, trialsinceswitch %in% asymp_trials), .(sID, Gender, age_group_, age_group, PreciseYrs, model_name), summarize, ACC=mean(ACC))

e_age_asymptbeh_validation =
  e_age_stayappswitch_validation + aes(y=100 * ACC)
e_age_asymptbeh_validation$data = dat

d_age_asymptbeh =
  d_age_stayappswitch + aes(y=100 * ACC)
d_age_asymptbeh$data = dat

# Effect of positive and negative outcomes (rewnorew_regr)
dat = subset(nback_regr_dat, model == "rewardNoReward" & back %in% 1:3 & predictor != "(Intercept)" & !is.na(age_group))
e_age_logregr_validation =
  e_age_stayappswitch_validation + aes(y=sig_Estimate) +
  facet_wrap(back ~ predictor, scales="free", nrow=3)
e_age_logregr_validation$data = dat

d_age_logregr = 
  d_age_stayappswitch + aes(y=sig_Estimate) +
  facet_wrap(back ~ predictor, scales="free", nrow=3)
d_age_logregr$data = subset(dat, model_name=="Human")

# Save plots
ggsave(file.path(plot_dir, "d_age_stayappswitch.png"), d_age_stayappswitch, width=6, height=3)
ggsave(file.path(plot_dir, "d_age_asymptbeh.png"), d_age_asymptbeh, width=6, height=3)
ggsave(file.path(plot_dir, "d_age_logregr.png"), d_age_logregr, width=6, height=7)

ggsave(file.path(plot_dir, "e_age_stayappswitch_validation.png"), e_age_stayappswitch_validation, width=4, height=3)
ggsave(file.path(plot_dir, "e_age_asymptbeh_validation.png"), e_age_asymptbeh_validation, width=4, height=3)
ggsave(file.path(plot_dir, "e_age_logregr_validation.png"), e_age_logregr_validation, width=6, height=7)
```
...
f_age_param =
  ggplot(param_dat_l, aes(age_group_, param_value, group=1)) +
...
```{r Model parameter regression}
# Across models
summary(lm(beta_RL ~ (beta_BF + persev_BF + p_switch + p_reward) ^ 2, data=param_dat))  # R^2 = 0.97
summary(lm(persev_RL ~ (beta_BF + persev_BF + p_switch + p_reward) ^ 2, data=param_dat))  # R^2 = 0.97
summary(lm(nalpha ~ (beta_BF + persev_BF + p_switch + p_reward) ^ 2, data=param_dat))  # R^2 = 0.93
summary(lm(alpha ~ (beta_BF + persev_BF + p_switch + p_reward) ^ 2, data=param_dat))  # R^2 = 0.25

summary(lm(beta_BF ~ (beta_RL + persev_RL + alpha + nalpha) ^ 2, data=param_dat))  # R^2 = 0.97
summary(lm(persev_BF ~ (beta_RL + persev_RL + alpha + nalpha) ^ 2, data=param_dat))  # R^2 = 0.97
summary(lm(p_reward ~ (beta_RL + persev_RL + alpha + nalpha) ^ 2, data=param_dat))  # R^2 = 0.91
summary(lm(p_switch ~ (beta_RL + persev_RL + alpha + nalpha) ^ 2, data=param_dat))  # R^2 = 0.79
...
# Explained variance
dat = data.frame(summary(param_pca)$importance)
dat$index = rownames(dat)
dat_l = melt(dat, id.vars="index")
g_explained_variance =
  ggplot(dat_l, aes(variable, value, group=1)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ index)

# PCs over age
PCs = cbind(data.frame(param_pca$x), param_dat)
...
pca_weights$index = rownames(pca_weights)
pca_weights

pca_weights_l = melt(pca_weights, id.vars="index")

ggplot(subset(pca_weights_l, variable %in% c("PC1", "PC2", "PC3", "PC4")), aes(index, variable, fill=value)) +
  geom_tile() +
  labs(x="", y="")

ggplot(subset(pca_weights_l, variable %in% c("PC1", "PC2", "PC3", "PC4")), aes(index, value)) +
  stat_summary(geom="bar") +
  # labs(x="", y="") +
  facet_wrap(~ variable)
```
```{r kmeans}
autoplot(kmeans(param_dat[param_names], 5), data=param_dat)
```
```{r kmeans}
library("cluster")
autoplot(clara(param_dat[param_names], 5))
autoplot(fanny(param_dat[param_names], 5), frame=T)
autoplot(pam(param_dat[param_names], 5), frame.type="norm")
```

```{r lfda}
library(lfda)
model = lfda(param_dat[param_names], param_dat["age_group"], r=6, metric="plain")
autoplot(model, data=param_dat, frame=T, frame.color="age_group")
```
