...
get_ages = function(ages_file_dir) {
  ...
  ages$log_T = log(ages$meanT)  # old log(ages$T134)
...
plot_sample_composition = function(all_files, gg_save, run_regression) {
  ...
  # Sample composition
  dat = ddply(all_files, .(sID, PDS, Gender, log_T, PreciseYrs, age_group, PDS_group, T_group), summarize, z=NA)
  dat$z = NULL
  dat$PDS[is.na(dat$PDS)] = NA  # replace NaNs with NAs for scale_fill_hue(na.value)
  dat$log_T[is.na(dat$log_T)] = NA  # replace NaNs with NAs for scale_fill_hue(na.value)
  
  # melt_dat = melt(dat, id.vars=c("sID", "Gender"))
  # gg_all_compositions =
  #   ggplot(melt_dat, aes(x=factor(1), fill=factor(round(value)))) + 
  #   scale_fill_hue(na.value = "grey50") +
  ...
  gg_age_composition =
    ggplot(dat, aes(x=factor(1), fill=factor(round(PreciseYrs)))) + 
    scale_fill_hue(na.value = "grey50") +
    geom_bar(width=1) + coord_polar("y") +
    labs(fill="Age") +
    theme(axis.text = element_blank(),
  ...
  gg_PDS_composition = gg_age_composition + aes(fill=factor(round(PDS*4)/4)) + labs(fill="PDS")
  gg_PDS_composition$data = subset(dat, PreciseYrs < 18)
  gg_T_composition = gg_PDS_composition + aes(fill=factor(round(log_T*3)/3)) + labs(fill="log. Testost.")
  
  age_comp = ggplot(dat, aes(age_group, fill=age_group)) +
    geom_bar() +
    scale_fill_manual(values=colors_3and1) +
    scale_color_manual(values=colors_3and1) +
    facet_grid(~ Gender)
  PDS_comp = age_comp + aes(PDS_group, fill=PDS_group)
  PDS_comp$data = subset(PDS_comp$data, (PreciseYrs < 18) & (sID != 410) & !is.na(PDS_group))
  T_comp = age_comp + aes(T_group, fill=T_group)
  T_comp$data = subset(T_comp$data, (PreciseYrs < 18) & (sID != 410) & !is.na(T_group))
  
  # gg_age_composition_f = gg_age_composition
  # gg_age_composition_f$data = subset(gg_age_composition$data, Gender=="Female")
  # 
  # gg_age_composition_m = gg_age_composition
  # gg_age_composition_m$data = subset(gg_age_composition$data, Gender=="Male")
  # 
  # gg_PDS_composition_f = gg_PDS_composition
  # gg_PDS_composition_f$data = subset(gg_PDS_composition$data, Gender=="Female")
  # 
  # gg_PDS_composition_m = gg_PDS_composition
  # gg_PDS_composition_m$data = subset(gg_PDS_composition$data, Gender=="Male")
  # 
  # gg_T_composition_f = gg_T_composition
  # gg_T_composition_f$data = subset(gg_T_composition$data, Gender=="Female")
  # 
  # gg_T_composition_m = gg_T_composition
  # gg_T_composition_m$data = subset(gg_T_composition$data, Gender=="Male")
  
  # Relationship between predictors
  # dat = subset(ages, sID < 300)
  gg_age_PDS =
    ggplot(subset(dat, (PreciseYrs < 18) & (sID != 410)), aes(PreciseYrs, PDS, color=Gender)) +
    geom_point(size=0.4) +
  ...
  if (gg_save) {
    if (data_name %in% c("human", "fitted_human")) {
      ggsave(file.path(plot_dir, "/gg_PDS_composition.eps"), gg_PDS_composition, width=16, height=8)
      ggsave(file.path(plot_dir, "/gg_age_composition.eps"), gg_age_composition, width=16, height=8)
      ggsave(file.path(plot_dir, "/gg_T_composition.eps"), gg_T_composition, width=20, height=9)
      ggsave(file.path(plot_dir, "/age_comp.eps"), age_comp, width=5, height=2)
      ggsave(file.path(plot_dir, "/PDS_comp.eps"), PDS_comp, width=5, height=2)
      ggsave(file.path(plot_dir, "/T_comp.eps"), T_comp, width=5, height=2)
      svg(file.path(plot_dir, "/yrs_age_T_pair_plot.eps"))
      pairs(ages[ages$sID < 300, c("PreciseYrs", "PDS", "log_T")], pch = 19, lower.panel=NULL, col=ages$Gender)
      dev.off()
      ggsave(file.path(plot_dir, "/gg_age_PDS.eps"), gg_age_PDS, width=3, height=2)
      ggsave(file.path(plot_dir, "/gg_age_T.eps"), gg_age_T, width=3, height=2)
      ggsave(file.path(plot_dir, "/gg_PDS_T.eps"), gg_PDS_T, width=3, height=2)
    }
  }
}
```
...
plot_performance_over_age = function(ACCs, gg_save) {
  ...
  n_subj = sqrt(length(unique(ACCs$sID)))
  ...
  # Correct over age
  gg_ACC_age =
    ggplot(ACCs, aes(PreciseYrs, mean_ACC, color=Gender, fill=Gender)) +
    geom_point()
    # geom_smooth()
  
  # Stay over age
  gg_stay_age =
    gg_ACC_age + aes(y=mean_stay) +
      # stat_function(fun = function(x) -0.586402 + 0.171191 * x -0.004136 * x ^ 2, color='black') +  # same as line above
      geom_ribbon(aes(ymin=stay_fit_low, ymax=stay_fit_hig), linetype=0, fill='black', alpha=0.05) +
      geom_line(aes(y=stay_fit), color='black') +
      coord_cartesian(ylim=c(0.35, 0.93))

  # Coins won across age
  gg_reward_age =
    gg_ACC_age + aes(y=mean_reward * 120) +
      geom_ribbon(aes(ymin=reward_fit_low, ymax=reward_fit_hig), linetype=0, fill='black', alpha=0.05) +  # 120 is number of trials
      geom_line(aes(y=reward_fit), color='black') +
      coord_cartesian(ylim=c(52, 80))
  
  # RTs across age
  gg_RT_age =
    gg_ACC_age + aes(y=median_cor_RT) +
  ...
  if (gg_save) {
    ggsave(file.path(plot_dir, "/gg_age_ACC.eps"), gg_ACC_age)
    ggsave(file.path(plot_dir, "/gg_age_stay.eps"), gg_stay_age)
    ggsave(file.path(plot_dir, "/gg_age_reward.eps"), gg_reward_age)
    ggsave(file.path(plot_dir, "/gg_age_RT.eps"), gg_RT_age)
    ...
    ggsave(file.path(plot_dir, "/gg_age_switch.eps"), gg_switch_age)
  }
}
...
plot_RTs_ACC_choice_rewards_stay = function(all_files, ACC_subj, ACC_subj_block, reward_subj, reward_subj_block, all_files_sum, gg_save) {
  ...
  # Choice over blocks
  gg_choice_blocks =
    ggplot(subset(all_files, !is.na(choice_left)),
    ...
    facet_grid(trial_block ~ age_group)
  ...
  gg_ACC_blocks_PDS$data = subset(gg_ACC_blocks$data, !is.na(PDS_group))
  ...
  gg_ACC_blocks_inset = ggplot(subset(ACC_subj, trialsinceswitch==1), aes(age_group, ACC, fill=age_group)) +
    scale_fill_manual(values=colors_3and1) +
    stat_summary(geom="bar") +
    stat_summary() +
    theme(legend.position = "none") +
    facet_grid(~ Gender)
  gg_ACC_blocks_inset_PDS = gg_ACC_blocks_inset + aes(x=PDS_group, fill=PDS_group)
  gg_ACC_blocks_inset_PDS$data = subset(gg_ACC_blocks_inset_PDS$data, !is.na(PDS_group))
  gg_ACC_blocks_inset_T = gg_ACC_blocks_inset + aes(x=T_group, fill=T_group)
  gg_ACC_blocks_inset_T$data = subset(gg_ACC_blocks_inset_T$data, !is.na(T_group))
  
  gg_ACC_block_blocks = gg_ACC_blocks + facet_grid(Gender ~ trial_block)
  gg_ACC_block_blocks$data = ACC_subj_block
  ...
  # Rewards over trials
  gg_rewards =
    ggplot(all_files, aes(TrialID, 100 * reward, fill = age_group)) +
    stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "bar") +
  ...
  gg_rewards2 =
    ggplot(subset(all_files_sum, !is.na(choice_12_back)),
  ...
  stay_plot_PDS$data = subset(stay_plot_PDS$data, !is.na(PDS_group))
  ...
  stay_plot_block = stay_plot + facet_grid(Gender ~ trial_block)
  stay_plot_block$data = reward_subj_block
  
  if (gg_save) {
    if (data_name %in% c("human", "fitted_human")) {
      ggsave(file.path(plot_dir, "/gg_RT.eps"), gg_RT)
      ggsave(file.path(plot_dir, "/gg_RTt.eps"), gg_RTt)
      ggsave(file.path(plot_dir, "/gg_RT_blocks.eps"), gg_RT_blocks, width = 8, height = 3)
    }
    ggsave(file.path(plot_dir, "/gg_ACC.eps"), gg_ACC)
    ggsave(file.path(plot_dir, "/gg_choice_blocks.eps"), gg_choice_blocks, width = 8, height = 1 + 2 * length(unique(gg_choice_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_rewards.eps"), gg_rewards)
    ggsave(file.path(plot_dir, "/gg_rewards2.eps"), gg_rewards2, width = 8, height = 1.4 + 2 * length(unique(gg_ACC_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_.eps"), gg_ACC_blocks, width = 3.3, height = 1.5 + length(unique(gg_ACC_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_ACC_block_blocks.eps"), gg_ACC_block_blocks, width = 5.5, height = 3)
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_PDS_.eps"), gg_ACC_blocks_PDS, width = 3.3, height = 1.5 + length(unique(gg_ACC_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_T_.eps"), gg_ACC_blocks_T, width = 3.3, height = 1.5 + length(unique(gg_ACC_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "/stay_plot_.eps"), stay_plot, width = 3.3, height = 2 + length(unique(stay_plot$data$model_name)))
    ggsave(file.path(plot_dir, "/stay_plot_block.eps"), stay_plot_block, width = 5, height = 3)
    ggsave(file.path(plot_dir, "/stay_plot_PDS_.eps"), stay_plot_PDS, width = 3.3, height = 2 + length(unique(stay_plot$data$model_name)))
    ggsave(file.path(plot_dir, "/stay_plot_T_.eps"), stay_plot_T, width = 3.3, height = 2 + length(unique(stay_plot$data$model_name)))
    
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_inset_PDS.eps"), gg_ACC_blocks_inset_PDS, width = 2, height = 2 + length(unique(stay_plot$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_inset_T.eps"), gg_ACC_blocks_inset_T, width = 2, height = 2 + length(unique(stay_plot$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_inset.eps"), gg_ACC_blocks_inset, width = 2, height = 2 + length(unique(stay_plot$data$model_name)))
  }
}
```
...
plot_switch_age_T_PDS = function(ACC_subj, reward_subj, run_regression, gg_save) {
  ...
  gg_ACC1trialswitch_age =
    ggplot(subset(ACC_subj, trialsinceswitch==1 & PreciseYrs < 18), aes(PreciseYrs, ACC, color=Gender)) +
    geom_point() +
  ...
  if (gg_save) {
    if (data_name %in% c("human", "fitted_human", "simulated_human")) {
      ggsave(file.path(plot_dir, "/gg_ACC1trialswitch_age.eps"), gg_ACC1trialswitch_age)
      ggsave(file.path(plot_dir, "/gg_ACC1trialswitch_PDS.eps"), gg_ACC1trialswitch_PDS)
      ggsave(file.path(plot_dir, "/gg_ACC1trialswitch_T.eps"), gg_ACC1trialswitch_T)
      ggsave(file.path(plot_dir, "/gg_stayrewnorew_PDS.eps"), gg_stayrewnorew_PDS)
      ggsave(file.path(plot_dir, "/gg_stayrewnorew_age.eps"), gg_stayrewnorew_age)
      ggsave(file.path(plot_dir, "/gg_stayrewnorew_T.eps"), gg_stayrewnorew_T)
      ggsave(file.path(plot_dir, "/gg_11to14_PDS.eps"), gg_11to14_PDS)
      ggsave(file.path(plot_dir, "/gg_11to14_T.eps"), gg_11to14_T)
...
plot_wsls = function(wsls, gg_save) {
  ...
  if (gg_save) {
    ggsave(file.path(plot_dir, "/gg_wsls.eps"), gg_wsls)
...
correlate_params_behavior = function(all_files, human_data_dir, model_name, gg_save) {
  ...
  if (grepl("B", model_name)) {
    params = ddply(all_files, .(sID),
                   PreciseYrs=PreciseYrs[1], PDS=PDS[1], BMI=BMI[1], meanT=meanT[1],
  ...
  # Exclude subjects
  excl_subj = plot_indivduals_for_exclusion(hum_files, gg_save, exclude_UCB)
  ...
  if (gg_save) {
    ...
    png(file.path(plot_dir, "/gg_corrplot.eps"), pointsize=15, width=600, height=600)
    ...
    eps(file.path(plot_dir, "/gg_corrplot.eps"), width=800, height=800)
    corrplot(cor_matrix, method="color", col=col(200),
         order="hclust", addrect=3, number.cex = 0.8, #type = "upper", 
         addCoef.col="black", # Add coefficient of correlation
         tl.col="black", tl.srt=30, cl.pos="b", # Text label, color, and rotation
  ...
  for (subj in unique(all_files_regr$sID)) {
    for (n in 1:12) {
      ...
      if (nrow(subj_dat) > 0) {
        ...
        # rew = glm("selected_box ~ back1reward", family="binomial", data=subj_dat)
        # rew$aic
        # nor = glm("selected_box ~ back1noReward", family="binomial", data=subj_dat)
        # nor$aic
        # rew_nor = glm("selected_box ~ back1reward + back1noReward", family="binomial", data=subj_dat)
        # rew_nor$aic
        # null = glm("selected_box ~ 1", family="binomial", data=subj_dat)
        # null$aic
...
# subset(nback_regr_dat, (sID == 49) & (back == 1))
# 
...
plot_regression = function(nback_regr_dat, gg_save) {
  ...
  rewnorew_dat = subset(nback_regr_dat, model == "rewardNoReward" & predictor != "(Intercept)" & `Pr...z..` < 0.99, select=c("PDS_group", "T_group", "age_group", "sID", "predictor", "back", "Estimate", "Gender"))
  # rewnorew_regr =
  ...
  # rewnorew_regr_PDS$data = subset(rewnorew_regr$data, !is.na(PDS_group))
  ...
  dat = melt(rewnorew_dat, measure.vars = c("PDS_group", "T_group", "age_group"), variable.name="group_name", value.name="group")
  dat$group = factor(dat$group, levels=c("25%", "50%", "75%", "100%", "UCB", "Adult"))
  rewnorew_regr_pos = ggplot(subset(dat, !is.na(group) & predictor == "reward"), aes(back, Estimate, color=group)) +
      stat_summary(fun.data = mean_se, geom = "errorbar", width=0) +
      stat_summary(fun.data = mean_se, geom = "line") +
      ...
      labs(x = "Trials back", y = "Regression weight", color = "") +
      facet_grid( ~ group_name)
  rewnorew_regr_neg = rewnorew_regr_pos
  rewnorew_regr_neg$data = subset(dat, !is.na(group) & predictor == "noReward")

  # dat = subset(nback_regr_dat, model == "rewardNoReward" & back %in% 1:5)
  # gg_regr_inset = ggplot(subset(dat, back==1 & predictor != "(Intercept)"), aes(age_group, Estimate, fill=age_group)) +
  #   stat_summary(geom="bar") +
  #   stat_summary() +
  #   scale_fill_manual(values=colors_3and1) +
  #   theme(legend.position="none") +
  #   facet_wrap(predictor ~ Gender, scales="free")
  # gg_regr_inset_PDS = gg_regr_inset + aes(x=PDS_group, fill=PDS_group)
  # gg_regr_inset_PDS$data = subset(gg_regr_inset_PDS$data, !is.na(PDS_group))
  # gg_regr_inset_T = gg_regr_inset + aes(x=T_group, fill=T_group)
  # gg_regr_inset_T$data = subset(gg_regr_inset_T$data, !is.na(T_group))

  if (gg_save) {
    ggsave(file.path(plot_dir, "gg_nback_regr_agecats.eps"), gg_nback_regr_agecats) #8, height=1 + 2 * length(unique(gg_ACC_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "gg_nback_regr_agecats_sig.eps"), gg_nback_regr_agecats_sig) #8, height=1 + 2 * length(unique(gg_ACC_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "gg_nback_regr_agecont.eps"), gg_nback_regr_agecont) #7, height=12)
    ggsave(file.path(plot_dir, "gg_nback_regr_agecont_sig.eps"), gg_nback_regr_agecont_sig) #7, height=12)
    # ggsave(file.path(plot_dir, "rewnorew_regr_age.eps"), rewnorew_regr, width=4, height=3) #5, height=3 + length(unique(gg_ACC_blocks$data$model_name)))
    # ggsave(file.path(plot_dir, "rewnorew_regr_PDS.eps"), rewnorew_regr_PDS, width=4, height=3) #5, height=3 + length(unique(gg_ACC_blocks$data$model_name)))
    # ggsave(file.path(plot_dir, "rewnorew_regr_T.eps"), rewnorew_regr_T, width=4, height=3) #5, height=3 + length(unique(gg_ACC_blocks$data$model_name)))
    ggsave(file.path(plot_dir, "rewnorew_regr_pos_.eps"), rewnorew_regr_pos, width=5.5, height=2)
    ggsave(file.path(plot_dir, "rewnorew_regr_neg_.eps"), rewnorew_regr_neg, width=5.5, height=2)

    # ggsave(file.path(plot_dir, "gg_regr_inset.eps"), gg_regr_inset, width=2.5, height=3.5)
    # ggsave(file.path(plot_dir, "gg_regr_inset_PDS.eps"), gg_regr_inset_PDS, width=2.5, height=3.5)
    # ggsave(file.path(plot_dir, "gg_regr_inset_T.eps"), gg_regr_inset_T, width=2.5, height=3.5)
  }
}
```
...
library("ggplot2"); theme_set(theme_classic()); library("plyr"); library("reshape2"); library("R.matlab"); library("zoo"); library(lmerTest)#; library("corrplot")#; library("Hmisc")
...
if (data_name == "simulated_human") {
  ...
  for (data_dir in data_dirs) {
    # data_dir = 'C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/simulate/simulations/RLabnp2_age_z_271'
    ...
    # Create summary data
    stuff = get_ACC_reward_subj(all_files)  # ACC_subj, ACC_subj_block, reward_subj, reward_subj_block, ACCs
    ACC_subj = stuff[[1]]
    ACC_subj_block = stuff[[2]]
    reward_subj = stuff[[3]]
    reward_subj_block = stuff[[4]]
    ACCs = stuff[[5]]
    ...
    plot_RTs_ACC_choice_rewards_stay(all_files, ACC_subj, ACC_subj_block, reward_subj, reward_subj_block, all_files_sum, gg_save)
...
```{r Summary analyses for models and humans}
# Get human data
plot_dir = file.path(human_data_dir, "PlotsWithoutUCB")
ages = get_ages(ages_file_dir)
all_files = get_data(human_data_dir, "human", ".*mat")
all_files = finish_all_files(all_files, ages)
excl_subj = plot_indivduals_for_exclusion(all_files, gg_save, exclude_UCB)
all_files = subset(all_files, !sID %in% excl_subj)
...
for (data_dir in data_dirs) {
  ...
  sim_all_files = finish_all_files(sim_all_files, ages)
  
  # Exclude subjects
  sim_excl_subj = plot_indivduals_for_exclusion(sim_all_files, gg_save, exclude_UCB)
...
```{r Plot}
# Plot
## "Apparent switch trials" (3rd bar of stay_plot)
gg_mod_app_switch = ggplot(subset(reward_subj, outcome_21_back=="reward, no reward"), aes(age_group, 100 * stay, group=model_name, color=model_name)) +
  stat_summary() +
  stat_summary(geom="line") +
  labs(x="", y="Stay after (app.) switch trial (%)", color="")# +
  # facet_grid(~ Gender)

## Asymptotic behavior (non-switch trials in gg_ACC_blocks)
asymp_trials = c(-3:-2, 3:20)
gg_mod_asymp_beh = ggplot(subset(ACC_subj, trialsinceswitch %in% asymp_trials), aes(age_group, 100 * ACC, group=model_name, color=model_name)) +
  stat_summary() +
  stat_summary(geom="line") +
  labs(x="", y="Asymptotic performance", color="") #+
  # facet_grid(~ Gender)

## Effect of positive and negative outcomes (rewnorew_regr)
dat = subset(nback_regr_dat, model == "rewardNoReward" & back %in% 1:5)
dat = subset(nback_regr_dat, model == "rewardNoReward" & back == 1)
# gg_mod_logregr =
  ggplot(subset(dat, predictor != "(Intercept)" & `Pr...z..` < 0.98 & !is.na(age_group)), aes(age_group, Estimate, group=model_name, color=model_name)) +
  stat_summary() +
  stat_summary(geom="line") +
  geom_hline(yintercept = 0, linetype = "dashed") +
...
if (gg_save) {
    plot_dir = file.path(human_data_dir, "PlotsWithUCB")
    ggsave(file.path(plot_dir, "gg_mod_app_switch.eps"), gg_mod_app_switch, width=4, height=3)
    ggsave(file.path(plot_dir, "gg_mod_asymp_beh.eps"), gg_mod_asymp_beh, width=4, height=3)
    ggsave(file.path(plot_dir, "gg_mod_logregr.eps"), gg_mod_logregr, width=7, height=3)
}
  
# # Debugging the regression results
# ggplot(subset(dat, back %in% 1 & predictor != "(Intercept)" & !is.na(age_group)), aes(age_group, Estimate, group=model_name, color=model_name)) +  #  & Estimate < 18
# stat_summary() +
# stat_summary(geom="line") +
# # geom_point(position="jitter") +
# geom_hline(yintercept = 0, linetype = "dashed") +
...
dat = subset(nback_regr_dat, model == "rewardNoReward" & back %in% 1:5)
print(summary(lm(Estimate ~ PreciseYrs + PreciseYrs2 + Gender + back, subset(dat, predictor == "reward"))))
print(summary(lm(Estimate ~ PreciseYrs + PreciseYrs2 + Gender + back, subset(dat, predictor == "noReward"))))

teen_dat = subset(nback_regr_dat, model == "rewardNoReward" & back %in% 1:5 & PreciseYrs < 18)
print(summary(lm(Estimate ~ PDS + Gender + back, subset(teen_dat, predictor == "reward"))))
print(summary(lm(Estimate ~ PDS + Gender + back, subset(teen_dat, predictor == "noReward"))))
print(summary(lm(Estimate ~ logTz + Gender + back, subset(teen_dat, predictor == "reward"))))
print(summary(lm(Estimate ~ logTz + Gender + back, subset(teen_dat, predictor == "noReward"))))
...
plot_parameters = function(fitted_param_dir, model, plot_appx) {
  ...
  if (gg_save) {
    ggsave(file.path(plot_dir, paste0("gg_params_ages_free_", model, plot_appx, ".eps")), gg_params_ages_free, width=8, height=8)
    ggsave(file.path(plot_dir, paste0("gg_params_ages_01_", model, plot_appx, ".eps")), gg_params_ages_01, width=8, height=4.5)
    ggsave(file.path(plot_dir, paste0("gg_params_ages_beta_", model, plot_appx, ".eps")), gg_params_ages_beta, width=8, height=2.5)
    ggsave(file.path(plot_dir, paste0("gg_params_ages_p_", model, plot_appx, ".eps")), gg_params_ages_p, width=8, height=2.5)
  }
}
...
RL_ML_dir = "C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/params_RLabnp2_age_z_271_pymc3.csv"
BF_ML_dir = "C:/Users/maria/MEGAsync/SLCN/PShumanData/fitting/map_indiv/new_ML_models/MCMC/clustermodels/params_Bbspr_age_z_271_pymc3.csv"
