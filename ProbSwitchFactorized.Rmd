...
plot_sample_composition = function(all_files, gg_save, run_regression) {
  ...
  # Sample composition
  dat = ddply(all_files, .(sID, PDS, Gender, log_T, PreciseYrs), summarize, z=NA)
  dat$z = NULL
  ...
  gg_age_PDS = ggplot(subset(dat, PreciseYrs < 18), aes(PreciseYrs, PDS, color=Gender)) +
    geom_point(size=0.4) +
  ...
  if (gg_save) {
    if (data_name %in% c("human", "fitted_human")) {
      ggsave(file.path(plot_dir, "/gg_PDS_composition.png"), gg_PDS_composition, width=16, height=8)
      ggsave(file.path(plot_dir, "/gg_age_composition.png"), gg_age_composition, width=16, height=8)
      ggsave(file.path(plot_dir, "/gg_T_composition.png"), gg_T_composition, width=20, height=9)
      # ggsave(file.path(plot_dir, "/gg_PDS_composition_f.png"), gg_PDS_composition_f, width=8, height=8)
      # ggsave(file.path(plot_dir, "/gg_age_composition_f.png"), gg_age_composition_f, width=8, height=8)
      # ggsave(file.path(plot_dir, "/gg_T_composition_f.png"), gg_T_composition_f, width=10, height=9)
      # ggsave(file.path(plot_dir, "/gg_PDS_composition_m.png"), gg_PDS_composition_m, width=8, height=8)
      # ggsave(file.path(plot_dir, "/gg_age_composition_m.png"), gg_age_composition_m, width=8, height=8)
      # ggsave(file.path(plot_dir, "/gg_T_composition_m.png"), gg_T_composition_m, width=10, height=9)
...
plot_RTs_ACC_choice_rewards_stay = function(all_files, ACC_subj, ACC_subj_block, reward_subj, reward_subj_block, all_files_sum, gg_save) {
  ...
  if (gg_save) {
    ...
    ggsave(file.path(plot_dir, "/gg_ACC_block_blocks.png"), gg_ACC_block_blocks, width = 5.5, height = 3)
    ...
    ggsave(file.path(plot_dir, "/stay_plot_block.png"), stay_plot_block, width = 5, height = 3)
    ...
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_inset_PDS.png"), gg_ACC_blocks_inset_PDS, width = 2, height = 2 + length(unique(stay_plot$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_inset_T.png"), gg_ACC_blocks_inset_T, width = 2, height = 2 + length(unique(stay_plot$data$model_name)))
    ggsave(file.path(plot_dir, "/gg_ACC_blocks_inset.png"), gg_ACC_blocks_inset, width = 2, height = 2 + length(unique(stay_plot$data$model_name)))
  }
}
```
...
plot_regression = function(nback_regr_dat, gg_save) {
  ...
  if (gg_save) {
    ...
    ggsave(file.path(plot_dir, "gg_regr_inset.png"), gg_regr_inset, width=2.5, height=3.5)
    ggsave(file.path(plot_dir, "gg_regr_inset_PDS.png"), gg_regr_inset_PDS, width=2.5, height=3.5)
    ggsave(file.path(plot_dir, "gg_regr_inset_T.png"), gg_regr_inset_T, width=2.5, height=3.5)
  }
}
```
...
all_files_regr$model_name = "no_model"
nback_regr_dat$model_name = "no_model"
...
ggplot(subset(reward_subj, outcome_21_back=="reward, no reward"), aes(age_group, 100 * stay, group=model_name, color=model_name)) +
  stat_summary() +
  stat_summary(geom="line") +
  facet_grid(~ Gender)

## Asymptotic behavior (non-switch trials in gg_ACC_blocks)
asymp_trials = c(-3:-2, 3:7)
ggplot(subset(ACC_subj, trialsinceswitch %in% asymp_trials), aes(age_group, 100 * ACC, group=model_name, color=model_name)) +
  stat_summary() +
  stat_summary(geom="line") +
  facet_grid(~ Gender)
...
dat = subset(nback_regr_dat, model == "rewardNoReward" & back %in% 1:5)
ggplot(subset(dat, back==1 & predictor != "(Intercept)"), aes(age_group, Estimate, group=model_name, color=model_name)) +
  stat_summary() +
  stat_summary(geom="line") +
  geom_hline(yintercept = 0, linetype = "dashed") +
