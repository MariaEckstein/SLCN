...
# Plot
(g_rollingNSwitch_ACC = ggplot(ACCs_sorted, aes(x=PreciseYrs)) + 
...
# rollmean(asypmt_dat_sorted$ACC, k=rolling_window_k, fill=NA)
...
# t.test(mean_ACC ~ age_group, data=subset(ACCs, age_group %in% c("UCB", "75%")))
# t.test(mean_ACC ~ age_group, data=subset(ACCs, age_group %in% c("Adult", "75%")))
# 
# Points won
print("Total points")
...
t.test(mean_reward ~ age_group, data=subset(ACCs, age_group %in% c("UCB", "75%")))
t.test(mean_reward ~ age_group, data=subset(ACCs, age_group %in% c("Adult", "75%")))
# 
...
# t.test(stay ~ age_group, data=subset(app_switch_dat, age_group %in% c("UCB", "75%")))
# t.test(stay ~ age_group, data=subset(app_switch_dat, age_group %in% c("Adult", "75%")))
# 
...
# t.test(ACC ~ age_group, data=subset(asypmt_dat_sorted, age_group %in% c("UCB", "75%")))
# t.test(ACC ~ age_group, data=subset(asypmt_dat_sorted, age_group %in% c("Adult", "75%")))
```

```{r Number of switch trials over age}

switch_dat = ddply(all_files, .(sID, PreciseYrs, Gender, PDS, meanT, age_group), summarize, mean_switches=mean(switch_trial))

(g = ggplot(switch_dat, aes(PreciseYrs, mean_switches, color=Gender))
  + geom_point()
  + geom_smooth()
)

(b_age_nswitches = ggplot(switch_dat, aes(age_group, 120 * mean_switches, group=1)) #, group=Gender, color=Gender))
...
ggsave(file.path(plot_dir, paste0(model_class, "b_age_nswitches.eps")), b_age_nswitches, width=4, height=2)
ggsave(file.path(plot_dir, paste0(model_class, "b_age_nswitches.png")), b_age_nswitches, width=4, height=2)
```

```{r Points won over age}

points_dat = ddply(all_files, .(sID, model_name, PreciseYrs, Gender, PDS, meanT, age_group), summarize,
                   total_points=sum(reward),
                   n_trials=max(TrialID))

(g = ggplot(points_dat, aes(PreciseYrs, total_points))
  + geom_point()
  + geom_smooth()
)

(b_age_points = ggplot(points_dat, aes(age_group, total_points, color=model_name, group=model_name))  #, group=Gender, color=Gender))
  + stat_summary(geom="point")
  + stat_summary(geom="line")
  + stat_summary(geom="pointrange")
  # + facet_grid(~ model_name)
...
ggsave(file.path(plot_dir, paste0(model_class, "b_age_points.eps")), b_age_points, width=4, height=2)
ggsave(file.path(plot_dir, paste0(model_class, "b_age_points.png")), b_age_points, width=4, height=2)
```
...
points_dat$PreciseYrs2 = points_dat$PreciseYrs ^ 2
points_dat$PDS2 = points_dat$PDS ^ 2
points_dat$meanT@ = points_dat$meanT ^ 2
```
...
## Total points won
summary(lm(total_points ~ PreciseYrs + PreciseYrs2 + Gender, points_dat))
