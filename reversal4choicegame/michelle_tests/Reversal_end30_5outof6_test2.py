            ...
            if abs(x -self.boxloca[i][0])<3 and abs(y -self.boxloca[i][1])<3:
                ...
                if self.dug == self.trial[0]:
                    if self.trialscore == 0:
                        ...
                        self.trialscore = 1
                        self.RunningTotal+=1
                        # here is the original running total key
                        if self.RunningTotal == self.revafter:
                            self.reverse_now = True
                            # here are all of the other possibiities for 5/6 correct
                        else:
                            if self.correct_1back == True and self.correct_2back == True and self.correct_3back == True and self.correct_4back == True and self.correct_5back == True:
                                self.reverse_now = True
                            elif self.correct_1back == True and self.correct_2back == False and self.correct_3back == True and self.correct_4back == True and self.correct_5back == True:
                                self.reverse_now = True
                            elif self.correct_1back == True and self.correct_2back == True and self.correct_3back == False and self.correct_4back == True and self.correct_5back == True:
                                self.reverse_now = True
                            elif self.correct_1back == True and self.correct_2back == True and self.correct_3back == True and self.correct_4back == False and self.correct_5back == True:
                                self.reverse_now = True
                            elif self.correct_1back == True and self.correct_2back == True and self.correct_3back == True and self.correct_4back == True and self.correct_5back == False:
                                self.reverse_now = True
                            else:
                                self.reverse_now = False
                # if you got this trial wrong, reset running total 
                else: 
                    self.RunningTotal = 0
                    
                # if you jsut won this trial, mark that for the next trial!! 
                if self.trialscore == 1: 
                    self.correct_1back = True 
                else:
                    self.correct_1back = False 

        print ("reverse now", self.reverse_now)
        print ("self rev after", self.revafter)
        print("running total", self.RunningTotal)
        print ("self correct 1 back", self.correct_1back)
        print ("self correct 2 back", self.correct_2back)
        print ("self correct 3 back", self.correct_3back)
        print ("self correct 4 back", self.correct_4back)
        print ("self correct 5 back", self.correct_5back)
...
# starting each trial 
    def init_trial(self):
        ...
        self.kiki.setH(0)
        self.presLoc = [0,1,2,3] # this is location... color or box??
        self.canMove = True
        self.digging = False
        self.atbox = -1
        self.addstar = -1
        ...
        # update all of these regardless of correct or not 
        if self.correct_4back == True:
            self.correct_5back = True
        if self.correct_3back == True:
            self.correct_4back = True 
        if self.correct_2back == True:
            self.correct_3back = True 
        if self.correct_1back == True:
            self.correct_2back = True
    ...
    def initgame(self):
        ...
        self.trial_num_phase = 0   
         # new variables for 5/6 tracking 
        self.reverse_now = False     
        self.correct_1back = False
        self.correct_2back = False
        self.correct_3back = False
        self.correct_4back = False
        self.correct_5back = False
