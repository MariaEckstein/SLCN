...
# 1. in order to set game to end after 30 trials if participants don't meet criterion
# in acquisition phase (AQ) or reversal phase (rev) 
# 2. in order tomake criterion 5/6  instead of 5 in a row
...
# code to start the game 
    ...
    def log(self):
        ...
        #self.file.write("%s\t" % self.choice_color)
        self.file.write("%s\t" % self.novel_box)
        self.file.write("%s\t" % self.previous_reward)
    	...
    	for visit_number in self.visit_color:
    		self.file.write("%s\t" % visit_number)
...
# starting each trial 
    def init_trial(self):
        if self.reverse_now == True and self.reversal == 0:
            self.previous_reward = self.correctbox
            self.reversal = 1
            self.RunningTotal = 0
            self.TrialScoreList = []
        ...
        self.trialscore = 0
        self.trial = [self.correctbox] # this is where they are matched! 
        #self.choice.color = [self.textures[self.trial]]
        self.pos = [0.,0.]
    ...
    def initgame(self):
        ...
        self.file.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t" % 
            ("pID","Age","Gender","TrialNumber","Reversal", "Target", 
            "Choice", "NovelBox", "PrevReward","Correct","TotalScore","RunningTotal", "Last6TrialsTotal", "TrialNumPhase", 
