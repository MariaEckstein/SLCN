...
%     end;
    ...
    list = input('List number (11,...,14;21,...,24;41,...44): '); %JYD changed to accomodate new lists for UCLA teen scan
    % version = input('version number: ');
    startcycle = 1;%input('Block number (type 1)? ');
end;
...
c1_key = KbName('left');%KbName('f');
c2_key = KbName('right');%KbName('j');
c_key = {c1_key c2_key};%{'f' 'j'};
...
% ME commented out to use with laptop
numDevices = 1;
n = 1;
inputDevice = [n n];
...
%shows up just fine, as does the whole FB display
    ...
    if realFMRI==1,
        while noresp,
            [keyIsDown,secs,keyCode] = PsychHID('KbCheck');%JYD needs to be 1 to collect responses from buttonbox, was 2
            if keyIsDown && noresp,
...
% %         Screen('DrawText',w,reminder',xcenter-300,ycenter-160, white);end;
    ...
    for trial=1:ntrials,
        ...
        while (GetSecs - resp_start_time < max_stim_duration) && noresp,
            [keyIsDown,secs,keyCode] = PsychHID('KbCheck'); %changed from 2 to 1
