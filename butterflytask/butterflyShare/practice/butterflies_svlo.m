...
if ~strcmp(curr_dir,'practice')
    fprintf('You must start the experiment from the practice directory. Go there and try again.\n');
else
...
subject_code = 77;%input('Enter subject code: ', 's');
condition = 'p';%input('Is this practice (p)? ','s');

Verbal='v';%input('Enter verbal FB (v or n): ', 's');
...
realFMRI = 0;%input('Is this a scan session? 1 if yes, 0 if no: ');
...
w=Screen('OpenWindow', screenNumber,[],[0 0 1024 1024]);
...
if strcmp(condition,'p'),
    ...
    noresp=1;
    while noresp,
            [keyIsDown,secs,keyCode] = PsychHID('KbCheck');%, inputDevice(1)); %KbCheck(inputDevice(1)); 
            if keyIsDown && noresp,
                noresp=0;
            end;
            WaitSecs(0.001);
...
%         Screen('FrameRect',w,[0 250 100],Rchoicerect,5);
        ...
        [keyIsDown,secs,keyCode] = PsychHID('KbCheck'); %KbCheck(inputDevice(2));
%             if keyIsDown && noresp &&
