...
gg_age_ACC = ggplot(sum_dat, aes(PreciseYrs, mean_ACC)) +
  geom_point() +
...
ggplot(wsls, aes(PreciseYrs, stay, color = reward)) +
  geom_point()

gg_age_wsls = ggplot(wsls_wide, aes(PreciseYrs, wsls)) +
  geom_point()
gg_PDS_wsls = gg_age_wsls + aes(x = round(PDS * 2) / 2)

summary(all_files$block)
...
gg_age_rewhist = ggplot(subset(rewhist_dat, !is.na(outcome_12_back)), aes(PreciseYrs, stay, color = outcome_12_back)) +
  stat_summary(fun.data = "mean_se", geom = "pointrange")
gg_PDS_rewhist = gg_age_rewhist + aes(x = round(PDS * 2) / 2)
