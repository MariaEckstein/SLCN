...
```{r Set up parameters}
data = "RL"  # can be "RL", "Bayes", or "human"
...
for(filename in filenames) {
  if (data == "human") {
    ...
    subj_file$sID = sID
    print(filename)
    subj_file = subset(subj_file, !is.nan(reward))
...
```{r Model stuff}
if (data != "human") {
  gg_trial_values = ggplot(all_files, aes(TrialID, values_l)) +  # , color = factor(correct_box), group = 1
  ...
  if (gg_save) {
    ...
    ggsave(file.path(plot_dir, "gg_gg_switch_values.png"), gg_switch_values)
  }
}
```
```{r}
