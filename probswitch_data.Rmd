...
# Questions:
# Why did my regression with 4 parameters not work? P(right) ~ right_reward + left_reward + right_noreward + left_noreward

# TD:
# exponential -> transform plot axes with 1 / (1 + exp(-k/5))
# plot exponential lines on top of each other
# fix the old 11 10 01 00 plot!
# add number of previous incorrect as a predictor in my models
...
```
```{r Run regression P(right) ~ prev_action * reward and save in nback_regr_dat}
...
```
```{r Beautify nback_regr_dat and plot relationship with age}
# Beautify
...
gg_nback_regr_choice_agecont =
  ggplot(subset(nback_regr_dat, predictor %in% c("left", "right") & back %in% seq(1, 12, 2)),
...
gg_nback_regr_reward_agecont$data = subset(nback_regr_dat, predictor %in% c("reward", "noReward") & back %in% seq(1, 12, 2) & Estimate < 15)
...
dat = subset(nback_regr_dat, sID == 17 & predictor == "left")
...
for (subj in unique(nback_regr_dat$sID)) {
  ...
  for (pred in unique(nback_regr_dat$predictor[nback_regr_dat$predictor != "(Intercept)"])) {
    ...
    exp_mod =
      try(nls(Estimate ~ k * exp(-lambda * back), data = subj_dat, start = list(k = .1, lambda = .1)), silent = T)
...
if (!data == "human") {
  ...
  gg_example_probs = gg_example_probs
