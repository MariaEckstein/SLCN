...
for(filename in filenames) {
  ...
  subj_file$choice_left = subj_file$key == 10
...
# ACC over blocks
gg_blockACC = ggplot(subset(all_files, !is.na(choice_left)), aes(trialsinceswitch, 100 * better_box_left, color = choice_left, group = choice_left)) +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "pointrange") +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "line") +
  coord_cartesian(x = c(0, 10)) +
  labs(x = "Trials since switch", y = "% choice left")
gg_blockACC
...
all_files$reward_bin = factor(all_files$reward_bin, levels = c("nn", "yn", "ny", "yy"))
# all_files[!is.na(all_files$reward_bin),]
ggplot(subset(all_files, !is.na(reward_bin)),
       aes(reward_bin, choice_left, fill = both_past_choices)) +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "bar") +
  stat_summary(fun.data = mean_cl_normal, fun.args = list(mult = 1), geom = "pointrange") +
  facet_grid(~ both_past_choices)

 
ggplot(subset(all_files_sum, !is.na(choice_12_back)),
       aes(outcome_12_back, 100 * choice, color = choice_12_back, group = choice_12_back)) +
  ...
  labs(x = "Reward history (2 trials)", y = "% left choice", color = "Previous choices")
